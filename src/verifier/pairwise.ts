...
import { extractScores, type ScoreDistribution } from './scoring.js';
import type { Rewards } from './tournament.js';

export const PROTOCOL_VERSION = 'assay-pairwise-xml-at20-v2';
...
Give a concise assessment of both candidates (at most 200 words). Reserve enough output space for the mandatory verdicts. Your response is incomplete without both verdict lines. Then end with exactly one verdict per candidate:
...
export class PairwiseVerifier {
  ...
  constructor(
    ...
    this.limiter = new RequestLimiter(this.config.concurrency);
    ...
    if (this.config.maxOutputTokens + 2048 >= this.config.contextWindowTokens) {
  ...
  private messages(task: string, a: string, b: string, criterion: Criterion) {
    const messages = [
      ...
      { role: 'user' as const, content: JSON.stringify({ task, candidate_A: a, candidate_B: b, criterion,
        required_output: 'Concise assessment, then exactly one <score_A> LETTER </score_A> and one <score_B> LETTER </score_B>. Each LETTER is uppercase A–T. Both verdicts are mandatory.',
      }) },
    ];
    ...
    const upperBound = Buffer.byteLength(JSON.stringify(messages)) + this.config.maxOutputTokens + 2048;
  ...
  private async evaluate(task: string, a: string, b: string, criterion: Criterion, repetition: number, signal?: AbortSignal): Promise<Evaluation> {
    ...
    // Archive before parsing so malformed responses remain inspectable.
    await this.onResponse?.({ criterionId: criterion.id, repetition, swapped, response });
    const scores = extractScores(response, this.config.minCapturedMass);
    return { criterionId: criterion.id, repetition, swapped, response,
      a: scores[swapped ? 'score_B' : 'score_A'],
      b: scores[swapped ? 'score_A' : 'score_B'],
    };
  }
